...
import { capitalizeFirstLetter } from "../../utils/calculateNextReportDate"
...
const getReportEmailTemplate = (
    ...
    const categoryList = topSpendingCategories.map((category: any) => 
        `<li> 
            ${category.name} - ${formatCurrency(category.amount)} (${category.percentage}%) 
        </li>`
    ).join("")

    const insightList = insights.map((insight: string) => 
        `<li> ${insight} </li>`
    ).join("")
           ...
           <table cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 0 10px rgba(0,0,0,0.05);">
             ...
             <tr>
               <td style="background-color: #f0f0f0; text-align: center; padding: 15px; font-size: 12px; color: #999;">
                 &copy; ${currentYear} ExpVision. All rights reserved.
               </td>
             </tr>
           </table>
         </td>
